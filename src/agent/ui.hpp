...
protected:
  wrapper() = delete;
  explicit wrapper(bool ok) : m_ok(ok) {}
...
public:
  explicit window(const char *name, bool *p_open = nullptr, ImGuiWindowFlags flags = 0)
      : wrapper(ImGui::Begin(name, p_open, flags)) {}
  ~window() { ImGui::End(); }
};

class tab_bar : public wrapper {
public:
  explicit tab_bar(const char *str_id, ImGuiTabBarFlags flags = 0) : wrapper(ImGui::BeginTabBar(str_id, flags)) {}
  ~tab_bar() {
...
public:
  explicit tab_item(const char *label, bool *p_open = nullptr, ImGuiTabItemFlags flags = 0)
...
static inline bool hot_key(const char *label, ImGuiKey &key, const ImVec2 &size = {100.0f, 25.f}) {
  ...
  auto SetToPressedKey = [](ImGuiKey &key) {
    ...
    auto it = std::find_if(keyMap.begin(), keyMap.end(), [](auto key) { return ImGui::IsKeyPressed(key.first); });
    if (it != keyMap.end()) {
      key = it->first;
      return true;
    } else return false;
