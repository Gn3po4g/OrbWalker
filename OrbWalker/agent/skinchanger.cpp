...
void Load() {
  auto &champions = *(vector<Champion *> *)(*(uintptr_t *)oChampionManager + 0x18);
  for(auto &champion : champions) {
    ...
    for(auto &i : skinsIds) {
      ...
      auto skinDisplayname = format("game_character_skin_displayname_{}_{}", name.get(), i);
      ...
      string translatedName = i > 0 ? TranslateString(skinDisplayname.data()) : name;
      ...
      auto champName = FNV(champion->championName());
...
void Update() {
  ...
  std::call_once(changeSkin, []() {
    if(config::currentSkin > 0) {
      auto &values{championsSkins[FNV(self->dataStack()->baseSkin.model)]};
  ...
  if(ImGui::IsKeyPressed(ImGuiKey_5)) {
    ...
    auto playerHash = FNV(self->dataStack()->baseSkin.model);
  ...
  if(!self->dataStack()->stack.empty()) {
    if(auto championName{FNV(self->dataStack()->baseSkin.model)};
  ...
  for(auto minion : span(minions->data, minions->size)) {
    auto hash = FNV(minion->dataStack()->baseSkin.model);
    auto playerHash = FNV(self->dataStack()->baseSkin.model);
    if(auto owner = minion->GetOwner(); owner) {
