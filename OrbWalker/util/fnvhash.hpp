...
// #define FNVC(str) (std::integral_constant<fnv::hash, fnv::hash_constexpr(str)>::value)

template <std::size_t N>
constexpr uint64_t FNVC(const char (&str)[N]) {
  return fnv::hash_constexpr(str);
};

inline uint64_t FNV(const char *str) { return fnv::hash_runtime(str); };