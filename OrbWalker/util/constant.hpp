constexpr auto objIndex = 0x10;
constexpr auto objTeam = 0x3C;
constexpr auto objPosition = 0x220;
constexpr auto objVisible = 0x310;
constexpr auto objMana = 0x340;
constexpr auto objTargetable = 0xEB0;
constexpr auto objHealth = 0x1058;
constexpr auto objSkinId = 0x1234;
constexpr auto objActionState = 0x13C0;
constexpr auto objAttackRange = 0x16B4;
constexpr auto objSpellCast = 0x2A20;
constexpr auto objManaCost = 0x2A78;
constexpr auto objBuff = 0x27C8;
constexpr auto objSpell = 0x30B8;
constexpr auto objCharData = 0x3590;
constexpr auto objDataStack = 0x35C0;
constexpr auto objName = 0x3858;